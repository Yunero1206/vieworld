...
import { ArrowRight, CalendarDays, Check, MessageCircle, Send } from 'lucide-react';
...
function ContextHall({ artistId, artistName, roomId, heading = 'Hall đang xem cùng nhau' }: { artistId: string; artistName: string; roomId: string; heading?: string }) {
  ...
  return <aside className="artist-context-hall" aria-label={`Hall của ${artistName} trong hoạt động này`}>
    ...
    {member ? <>
      <div className="artist-context-hall-messages" role="log" aria-label="Trò chuyện cùng Hall">
        {publicDemo.slice(0, 3).map(voice => <p key={voice.id}><strong>{voice.author} · mẫu</strong><span>{voice.text}</span></p>)}
        {messages.slice(-12).map(item => <p key={item.id}><strong>{item.authorName}</strong><span>{item.text}</span></p>)}
...
export function ContextStage({ session, artistId, artistName, image }: { session: Session; artistId: string; artistName: string; image: string }) {
  ...
  return <section className="artist-context-stage" aria-labelledby="artist-context-title">
    ...
    <div className="artist-context-stage-grid"><div className="artist-context-main">
      {liveStage ? <div className="artist-context-live-media"><AvatarStage avatar={avatar} artistPresence={session.artistPresence} compact stageVariant="standard" /><SilentMediaPlaceholder isMuted={isMuted} onToggleMute={() => setIsMuted(value => !value)} isPlaying={isPlaying} onTogglePlay={() => setIsPlaying(value => !value)} mediaStatus={session.mediaStatus || 'cleared_local'} trackTitle="Âm thanh demo · chỉ phát khi bạn chọn" /></div>
