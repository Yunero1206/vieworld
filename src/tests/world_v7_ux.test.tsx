...
describe('v7 fan-first information architecture',()=>{
 ...
 it('custom deep link provides typed filters without publishing anything',()=>{mount('/me?custom=ticket');expect(screen.getByRole('searchbox',{name:'Tìm trong bộ sưu tập'})).toBeInTheDocument();expect(screen.getByRole('button',{name:'Vé'})).toHaveAttribute('aria-pressed','true');expect(screen.getByText('Bộ lọc')).toBeInTheDocument();});
